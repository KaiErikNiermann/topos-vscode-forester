...
- **Go-to-definition / Ctrl+click** on `\transclude{id}`, `\ref{id}`, `[text](id)`, and `[[id]]` links — resolving both `id.tree` files and inline `\subtree[id]{…}` declarations inside a parent file.
